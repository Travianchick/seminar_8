...
def search_data(contacts: list[str]): # Функция поиска данных
    # founded = [] # список, для блока с занесением пользователей , подходящих под поиск
    ...
    if search_idx == '0':
        ...
        return
    # Блок с занесением в список пользователей , подходящих под поиск
    ...
    #return founded
    # Блок с печатью пользователей из поиска с указанием номера их строки
    flag = False
    for idx, contact in enumerate(contacts):
        if search_str.lower() in contact.split(', ')[int(search_idx)].lower():
            print(idx+1, contact)
            flag = True
    if flag == False:
            print('Ни одного пользователя по запросу не найдено') 
